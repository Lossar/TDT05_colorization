...
hsv_images = np.empty((len(target), 3, 32, 32))

for image in range(len(target)):
    current = array_to_image(target[image])
    for i in range(32):
        for j in range(32):
            hsv_images[image][0][i][j], hsv_images[image][1][i][j], hsv_images[image][2][i][j] = colorsys.rgb_to_hsv(current[0][0][i][j], current[0][1][i][j], current[0][2][i][j])
...
for image in range(len(hsv_images)):
    for i in range(32):
        for j in range(32):
            hsv_images[image][0][i][j] *= 255
            hsv_images[image][1][i][j] *= 255
            hsv_images[image][2][i][j] *= 255
